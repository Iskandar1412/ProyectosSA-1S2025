<script>
    ...
    async function obtenerPromocionesProductos() {
        try {
            const resProductos = await fetch(`${pathComprasMS}/compras`, {
                method: 'GET',
                headers: {
            ...
            {#each productos as producto}
                <tr>
                    ...
                    <td class="acciones-td">
                        <button onclick={() => iniciarEdicion(producto)} class="btn-editar" title="Ver">
                            ✏️
                        </button>
                    </td>
                </tr>
...
<div class="modal-overlay {$darkMode ? 'dark-mode' : ''}">
    <div class="modal-content">
        ...
        <h3>Orden [ {productoEditando.id_orden} ]</h3>
